...
class TestLessonBuy(object):
    ...
    def setup_class(cls):
        ...
        pwd = '123456'
        typ = "mobile"
        cls.trial = cls.user.get_token(typ=typ,u=cls.config['shoppingtab']['ge_9.9_notpaid'],p=pwd)  # 体验课
        ...
        # 体验课
        cls.buy_trial = ApiSuper(cls.trial, agent)
        # # 英语大包课
    ...
    def test_buy_enBag(self):
        ...
        res = self.buy_trial.api_get_lessonbuy(self.source, "F1_S1-6_WITHDISCOUNT")
        print(res)
        # 断言接口返回成功
        assert res['code'] == 0
    ...
    def test_buy_formal_03(self):
        """3.0课程购买详情页_四周课"""
        res = self.buy_trial.api_get_lessonbuy(self.source, "S1GE_W1_4_SPU_new")
        print(res)
        # 断言接口返回成功
        assert res['code'] == 0
        # 断言详情页展示的spu是：S1GE_W1_4_SPU_new
        assert res['data']['display_items'][0]['_id'] == 'S1GE_W1_4_SGU_new'

    def test_buy_formal_01(self):
        """1.5课程详情页"""
        # res = self.buy_formal_01.api_get_lessonbuy(self.source, "L2XX_L2XX_L3XX")
        res = self.buy_trial.api_get_lessonbuy(self.source, "L2XX_L2XX_L3XX")
        print(res)
        # 断言接口返回成功
        assert res['code'] == 0
    ...
    def test_buy_formal_02(self):
        ...
        res = self.buy_trial.api_get_lessonbuy(self.source, "K2GE")
        print(res)
        # 断言接口返回成功
        assert res['code'] == 0
        # 断言详情页展示的spu是：K2GE
    ...
    def test_buy_ggr(self):
        ...
        res = self.buy_trial.api_get_lessonbuy(self.source, "GGRVIP_Purchase_01")
        print(res)
        # 断言接口返回成功
        assert res['code'] == 0
    ...
    def test_buy_expand(self):
        ...
        res = self.buy_trial.api_get_lessonbuy(self.source, "AlbumCIX001")
        print(res)
        # 断言接口返回成功
        assert res['code'] == 0
    ...
    def test_buy_disney(self):
        ...
        res = self.buy_trial.api_get_lessonbuy(self.source, "BundleCDS001")
        print(res)
        # 断言接口返回成功
        assert res['code'] == 0
    ...
    def test_buy_ma(self):
        ...
        res = self.buy_trial.api_get_lessonbuy(self.source, "K3MAFC_0_APP")
        print(res)
        # 断言接口返回成功
        assert res['code'] == 0
        # 断言详情页展示的spu是：K3MATC_99
        assert res['data']['display_items'][0]['_id'] == 'K3MAFC_0_SGU'

    def test_buy_maBag(self):
        """思维大包课"""
        # res = self.buy_maBag.api_get_lessonbuy(self.source, "K1MA_K6MA")
        res = self.buy_trial.api_get_lessonbuy(self.source, "K1MA_K6MA")
        print(res)
        # 断言接口返回成功
        assert res['code'] == 0

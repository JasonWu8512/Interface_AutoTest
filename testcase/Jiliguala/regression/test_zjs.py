...
class Testpossessweeks(object):
    ...
    def test_ZJS(self):
        ...
        resp2 = self.possessweeks1.api_order_create_V2(itemid=self.CS_user["itemid1"],url=self.CS_user["url4"], sp2xuIds=self.CS_user["sp2xuIds3"],source=self.CS_user["source2"],
                                                       sharer=self.CS_user["sharer"],xshareInitiator=self.CS_user["xshareInitiator"])
        assert resp2["code"] == 0
        # 取订单号传入下一个订单信息接口
        ...
        # 传入oid获取商品信息
        resp3 = self.possessweeks1.api_order_charge_V2(order=oid, extra=self.CS_user["extra4"])
        assert resp3["code"] == 0
        print(resp3)

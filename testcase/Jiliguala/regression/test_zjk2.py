...
class Testpossessweeks(object):
    ...
    def test_Zjk2(self):
        ...
        resp2 = self.possessweeks1.api_eshop_v2_orders2(payPrice=self.spu["payPrice2"],sp2xuId=self.spu["sp2xuId3"],spuNo=self.spu["spuNo3"])
        assert resp2["code"] == 0
        # 取订单号传入下一个订单信息接口
        ...
        # 传入oid获取商品信息
        resp3 = self.possessweeks1.api_eshop_v2_orders_charge(oid=oid, agent=self.spu["agent2"],
                                                              payTotal=self.spu["payTotal3"],
                                                              channel=self.spu["channel1"])
        assert resp3["code"] == 0
        ...
        # 断言购买的课程
        assert resp5["data"]["itemid"] == "K4GE" or "S4GE"
        print(resp5)

        # 页面跳转到支付成功页
        resp6 = self.possessweeks1.api_eshop_ghs_info(orderId=oid)
        assert resp6["data"]["title"] == "呱呱英语素质启蒙课K4"

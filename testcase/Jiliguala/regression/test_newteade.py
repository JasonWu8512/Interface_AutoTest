...
class Testpossessweeks(object):
    ...
    def test_newteade1(self):
        ...
        resp2 =self.possessweeks1.api_eshop_v2_orders(sp2xuid=self.spu["sp2xuId"],spuNo=self.spu["spuNo"])
        assert resp2["code"] == 0
        #取订单号传入下一个订单信息接口
        ...
        #传入oid获取商品信息
        resp3 = self.possessweeks1.api_eshop_v2_orders_charge(oid=oid,agent=self.spu["agent"],payTotal=self.spu["payTotal"],channel=self.spu["channel"])
        assert resp3["code"] == 0
        assert resp3["data"]["subject"] == "四周定制英语启蒙课" or "呱呱英语启蒙K1（4周）"
        print(resp3)
        ...
        #断言购买的课程
        assert resp5["data"]["itemid"] == "S1GE_W1_4_SGU_new"
        print(resp5)

...
class Testpossessweeks(object):
    ...
    def test_newteade1(self):
        ...
        resp2 = self.possessweeks1.api_eshop_v2_orders2()
        assert resp2["code"] == 0
        # 取订单号传入下一个订单信息接口
        ...
        # 传入oid获取商品信息
        resp3 = self.possessweeks1.api_eshop_v2_orders_charge(oid=oid)
        assert resp3["code"] == 0
        ...
        # assert resp6["data"]["title"] == "四周定制英语启蒙课"
        ...
        # 退款
        resp7 = self.refund.api_post_refund(orderNo=order_no)
        # assert resp7["code"] == 0
        assert resp7["msg"] == "success"
        print(resp7)

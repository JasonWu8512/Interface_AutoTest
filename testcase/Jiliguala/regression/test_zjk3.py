...
class Testpossessweeks(object):
    ...
    def test_Zjk3(self):
        ...
        resp2 = self.possessweeks1.api_eshop_v2_orders2(payPrice=self.spu["payPrice1"],sp2xuId=self.spu["sp2xuId2"],spuNo=self.spu["spuNo2"])
        assert resp2["code"] == 0
        # 取订单号传入下一个订单信息接口
        ...
        # 传入oid获取商品信息
        resp3 = self.possessweeks1.api_eshop_v2_orders_charge(oid=oid, agent=self.spu["agent"],
                                                              payTotal=self.spu["payTotal1"],
                                                              channel=self.spu["channel"])
        assert resp3["code"] == 0
        assert resp3["data"]["subject"] == "【班主任老用户专享②】3岁以上宝贝的素质启蒙K1-K6（不含学习"
        print(resp3)
        ...
        # 断言购买的课程
        assert resp5["data"]["itemid"] == "BZR_GE_KC_S1_S6_CG_2DC"
        print(resp5)

        # 页面跳转到支付成功页
        resp6 = self.possessweeks1.api_eshop_ghs_info(orderId=oid)
        assert resp6["data"]["title"] == "【班主任老用户专享②】3岁以上宝贝的素质启蒙K1-K6（不含学习材料Ⅰ）"

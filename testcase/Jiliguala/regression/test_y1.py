...
class Testpossessweeks(object):
    ...
    def test_Tfyk(self):
        ...
        resp2 = self.possessweeks1.api_eshop_v2_orders2(payPrice=self.spu["payPrice"],sp2xuId=self.spu["sp2xuId1"],spuNo=self.spu["spuNo1"])
        assert resp2["code"] == 0
        # 取订单号传入下一个订单信息接口
        ...
        resp3 = self.possessweeks1.api_eshop_v2_orders_charge(oid=oid, agent=self.spu["agent"],
                                                              payTotal=self.spu["payTotal"],
                                                              channel=self.spu["channel"])
        assert resp3["code"] == 0
        assert resp3["data"]["subject"] == "呱呱英语体系全年伴学班-Y1" or "【测试】年课Y1"
        print(resp3)
        ...
        # 断言购买的课程
        assert resp5["data"]["itemid"] == "Y1GE_SGU_new"
        print(resp5)

        # 页面跳转到支付成功页
        resp6 = self.possessweeks1.api_eshop_ghs_info(orderId=oid)
        # assert resp6["data"]["title"] == "四周定制英语启蒙课" or "呱呱英语启蒙K1（4周）"

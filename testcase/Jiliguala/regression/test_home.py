...
class TestHome(object):
    ...
    def test_home_unPurchased01(self):
        """
        未购用户，首页标题、周主题 展示无误
        """
    ...
    def test_home_exEn01(self):
        """
        已购体验课用户，英语tab，首页标题、周主题
        """
    ...
    def test_home_en01(self):
        ...
        assert res['code'] == 0
        # 断言首页标题展示：呱呱爱表达*
        assert res['data']['roadmap']['info']['ttl'] in '呱呱爱表达(2021)' or '呱呱爱表达'
        if self.env == 'fat':
            weekTitle = res['data']['roadmap']['elements'][0]['weekTitle']
            print(weekTitle)
        else:
            weekTitle = res['data']['roadmap']['elements'][1]['weekTitle']
            print(weekTitle)
        preWeekTitle = "第 1 周 / 共 24 周"
        # 断言顶部资源位
        if weekTitle == preWeekTitle:
            assert res['data']['roadmap']['elements'][0]['tips'][0]['title'] == '0801-英语TAB-4岁以上英语未购期外可扩科思维K3用户'
            assert res['data']['roadmap']['elements'][0]['tips'][1]['title'] == '230915学习TAB-转介绍-Banner'
            # 断言学习进度第x/共24周
            assert " 共 24 周" in res['data']['roadmap']['elements'][1]['weekTitle']
            # 断言有底部资源位
            assert res['data']['roadmap']['elements'][1]['tips'][0]['title'] == '拓展计划'
        else:
            print("正价课已开课")
    ...
    def test_home_enMix01(self):
        """
        用户已购英语体验课+正价课，英语tab，首页标题、周主题 展示无误
        """
        ...
        assert res['code'] == 0
        # # 断言首页标题展示：呱呱爱表达K1
        assert res['data']['roadmap']['info']['ttl'] == '呱呱爱表达(2021)' or '呱呱爱表达'
        assert res['data']['roadmap']['info']['level'] == 'K1'
        # 断言顶部资源位：展示填写地址资源位
        # assert res['data']['roadmap']['elements'][0]['tips'][0]['title'] == '正式课课前准备'
        # assert res['data']['roadmap']['elements'][0]['tips'][1]['title'] == '引导关注公众号'
    ...
    def test_home_maMix01(self):
        """
        用户已购（英语+思维）体验课+正价课，思维tab，首页标题、顶部资源位（已开课无资源位）、周主题 展示无误
        """
    ...
    def test_home_yt01(self):
        """
         用户已购年课体验课，英语tab，首页标题、周主题 展示无误
        """
        res = self.home_yt.api_get_v4_home(bid=self.b_yt_user)
        print(res)
        # # 断言接口返回成功
        assert res['code'] == 0

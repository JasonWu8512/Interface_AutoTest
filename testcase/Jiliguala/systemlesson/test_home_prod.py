...
class TestHome(object):
    ...
    def setup_class(cls):
        ...
        cls.home_en = ApiHome(cls.en_user, version)
        cls.home_ma = ApiHome(cls.ma_user)
        cls.home_mix = ApiHome(cls.mix_user, version)
    ...
    def test_home_unPurchased(self):
        ...
        # 断言顶部资源位展示无误
        assert res['data']['roadmap']['elements'][0]['tips'][0]['title'] == '了解体验课'
        # 断言周主题展示成：体验营
    ...
    def test_home_en(self):
        ...
        assert res['code'] == 0
        # 断言首页标题展示：呱呱爱表达*
        assert res['data']['roadmap']['info']['ttl'] in '呱呱爱表达(2020)'
    ...
    def test_home_enMix(self):
        ...
        assert res['code'] == 0
        # # 断言首页标题展示：呱呱爱表达K1
        assert res['data']['roadmap']['info']['ttl'] == '呱呱爱表达(2020)'
        assert res['data']['roadmap']['info']['level'] == '级别1'
        # 断言顶部资源位：展示填写地址资源位
        # assert res['data']['roadmap']['elements'][0]['tips'][0]['title'] == '正式课课前准备'
        # assert res['data']['roadmap']['elements'][0]['tips'][1]['title'] == '引导关注公众号'
        # 断言顶部资源位：专享试听内容
    ...
    def test_home_diploma(self):
        ...
        assert res['data']['roadmap']['elements'][1]['diploma'][
                   'image'] == 'https://qiniucdn.jiliguala.com/sc/graduationnew/image_graduation_available.png'
        # 断言有学习下一级别按钮
